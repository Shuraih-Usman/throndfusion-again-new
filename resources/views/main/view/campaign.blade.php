    ...
    <div class="container">
        <div class="row justify-content-center">
            ...
            <div class="col-lg-10 col-12">
                <div class="section-title-wrap mb-5">
                    <h4 class="section-title">{{$row->title}} by <a href="/account/{{$row->username}}">{{$row->username}} </a> </h4>
                </div>
        ...
        <div class="col-sx-12 col-sm-12 col-md-8 col-lg-8  pb-3">
            <div class="card border-0 shadow h-100">
                <img src="/images/{{$row->img_folder.$row->image}}" alt="{{$row->title}}" class="card-img-top">
                <div class="card-body pt-40px pb-30px">
                    <div class="d-flex justify-content-between">
                        <div class="text-center mb-2">
                            <span class="text-muted small">{{$row->created_at->format('D d M, Y')}}</span>
                        </div>
                        <div class="">
                    ...
                    <h4 class="mb-3 text-center">{{$row->title}}</h4>
                    <p> By <a href="/account/{{$row->username}}">{{$row->username}} </a> </p>
                    ...
                    <div class="mt-3 d-flex flex-column align-items-center">
                        ...
                        <a class="btn btn-primary mb-3 w-50" href="/message?service={{$row->id}}&user={{$row->user_id}}">
                            ASK QUESTION
                        </a>
                    ...
                    <div id="service_message_user" class="iziModal" data-izimodal-title="{{$row->title}}" data-izimodal-subtitle="Messaging {{$row->username}}">
                        ...
                        <div class=" card p-4">
                            <div class="row">
                                <form id="service_message" method="post">
                                    @csrf
                                <div class="col-12">
                                    <textarea class="form-control" name="message" id="urmessage"> Hi {{$row->username}} i want to know more about this service :  {{$row->title}}  </textarea>
                                </div>
            ...
            <div class="card border-0 shadow  sticky-top">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center justify-content-center">
                        ...
                        <input type="hidden" id="service_price" class="form-control" value="{{$row->price}}">
                        <input type="hidden" id="service_id" class="form-control" value="{{$row->id}}">
                        ...
                        <input type="hidden" id="tite_name" class="form-control" value="{{$row->title}}">
                        <input type="hidden" id="user_email" class="form-control" value="{{Admin('email')}}">
                    </div>
                    <div class="d-flex flex-column align-items-center">
                    <p class="mt-3"> Total : <span class="" id="total_service">{{$row->price}}</span></p>
